...
export async function renderHomeCity(
  city: City,
  ...
  const [areas, result, counts] = await Promise.all([
    fetchAreaChips(city.id),
    fetchBrokers(filters),
    fetchFacetCounts(q, city.id),
  ]);
  ...
  return (
    <div>
      ...
      <div className="mt-4 flex items-center justify-between">
        <h1 className="display-1 text-2xl">Rental brokers in {city.name}</h1>
        <Link href={addHref} className="btn btn-primary btn-sm hidden sm:inline-flex">
          + Add
        </Link>
      </div>
      ...
      {initialBrokers.length === 0 ? (
        <EmptyState
          filtered={filtered}
          activeAreas={activeAreas.map((a) => a.name)}
          firstAreaSlug={activeAreas[0]?.slug}
          addHref={addHref}
        />
      ) : (
        <BrokerGrid
      ...
      <Link href={addHref} className="fab sm:hidden" aria-label="Add a broker">
        +
      </Link>
    </div>
  );
    ...
    <div className="broker-card mt-6 flex flex-col items-center gap-2 px-6 py-14 text-center">
      ...
      {filtered && areaName ? (
        <>
          ...
          <Link href={`${addHref}?area=${firstAreaSlug}`} className="btn btn-primary mt-2">
            Be the first to add
          </Link>
        </>
      ) : (
        <>
          ...
          <Link href={addHref} className="btn btn-primary mt-2">
            + Add a broker
          </Link>
        </>
      )}
